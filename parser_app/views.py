    ...
    def post(self, request):
        ...
        def process_resume(file):
            try:
                ...
                # 3.1) Generate ALL job summaries in ONE call (batched)
                if parsed_data.get("work_experience"):
                    try:
                        summaries = generate_individual_work_summaries(
                            parsed_data["work_experience"],
                            raw_text
                        )
                        parsed_data["work_experience_summaries"] = summaries
                    except Exception as e:
                        parsed_data["work_experience_summaries"] = [{"error": str(e)}]
